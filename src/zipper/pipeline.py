...
from src.zipper.core import SampleEntry, oov_replacement_vocabulary, letter_replacements
...
def replace_letter_to_sound(line):
    for letter, sound in letter_replacements.items():
        line = re.sub(fr"({__word_start_regex})[{letter.lower()}{letter.upper()}]+({__word_end_regex})",
                      fr"\1{sound}\2", line)
    return line


def test_replace_letter_to_sound():
    assert replace_letter_to_sound(
        "aAa b C d e f g h I J k L M n") == "aã bė̃ cė̃ dė̃ eẽ ef̃ gė̃ hãš iĩ jòt kã el̃ em̃ eñ"
